...
import { ensureBlogSeedData, ensureLeadSeedData } from "@/lib/portal-seed";
...
async function getDashboardSnapshot() {
  ...
  if (!supabase) {
    return {
      ...
      followUpCount: adminLeadRows.filter((lead) => lead.status === "Follow-up").length,
      leadRows: adminLeadRows.map((lead) => ({
      ...
      qualifiedLeadCount: adminLeadRows.filter((lead) => lead.status === "Qualified").length,
    };
  }

  await Promise.all([ensureLeadSeedData(supabase), ensureBlogSeedData(supabase)]);

  const [{ data: leads }, { count: blogCount }] = await Promise.all([
    supabase
      .from("lead_submissions")
      .select("id, full_name, company_name, email, source, status", { count: "exact" })
  ...
  const leadRows =
    leads?.map((lead) => ({
      ...
      name: lead.full_name,
      source: toTitleCase(lead.source),
      status: lead.status,
    })) ?? [];

  return {
    blogCount: blogCount ?? blogPosts.length,
    followUpCount: leadRows.filter((lead) => lead.status === "follow_up" || lead.status === "contacted").length,
    leadRows,
    qualifiedLeadCount: leadRows.filter((lead) => lead.status === "qualified").length,
  };
}

export default async function AdminDashboardPage() {
  const { blogCount, followUpCount, leadRows, qualifiedLeadCount } = await getDashboardSnapshot();
  ...
  const stats = [
    ...
    },
    {
      ...
      detail: "Needs qualification or next action",
    },
    {
  ...
  const oversightRows = [
    {
      ...
      context: leadRows[0]?.company ?? "Lead queue",
    ...
    {
      task: "Notification and escalation review",
      context: "Operations controls",
      schedule: "Unread alerts and routing rules",
      owner: "Notifications",
    ...
    },
    {
      ...
      schedule: "Ticket response and access issues",
      owner: "Support",
      href: "/admin/support",
      status: "Queued",
    },
          ...
          <div className="mt-10 space-y-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Link
                ...
                className="rounded-[1.25rem] border border-black/5 bg-white p-5 transition hover:-translate-y-1 hover:shadow-[0_14px_40px_rgba(15,23,42,0.08)]"
              >
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#8a0917]">Alerts & control</p>
                <h3 className="mt-3 text-xl font-semibold tracking-tight text-slate-950">Review notification flow</h3>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Stay ahead of client hub alerts, support escalations, and hiring workflow signals from
                  a dedicated operational inbox.
                </p>
              </Link>
              ...
              <Link
                ...
                className="rounded-[1.25rem] border border-black/5 bg-white p-5 transition hover:-translate-y-1 hover:shadow-[0_14px_40px_rgba(15,23,42,0.08)]"
              >
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#8a0917]">Support desk</p>
                <h3 className="mt-3 text-xl font-semibold tracking-tight text-slate-950">Handle client requests</h3>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Manage access questions, scheduling issues, and shared-document requests without losing
                  visibility across active client work.
                </p>
              </Link>
            </div>
